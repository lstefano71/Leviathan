...
internal sealed class CsvRecordDetailDialog : Window
{
    internal CsvRecordDetailDialog(long rowNumber, (string Name, string Value)[] fields)
    {
        ...
        content.MouseEvent += (_, e) => {
            if (e.Flags.HasFlag(MouseFlags.WheeledUp)) { content.ScrollVertical(-3); e.Handled = true; } else if (e.Flags.HasFlag(MouseFlags.WheeledDown)) { content.ScrollVertical(3); e.Handled = true; }
        };

        content.KeyDown += (_, e) => {
            if (e == Key.CursorUp) { content.ScrollVertical(-1); e.Handled = true; } else if (e == Key.CursorDown) { content.ScrollVertical(1); e.Handled = true; } else if (e == Key.CursorLeft) { content.ScrollHorizontal(-1); e.Handled = true; } else if (e == Key.CursorRight) { content.ScrollHorizontal(1); e.Handled = true; } else if (e == Key.PageUp) { content.ScrollVertical(-content.Viewport.Height); e.Handled = true; } else if (e == Key.PageDown) { content.ScrollVertical(content.Viewport.Height); e.Handled = true; } else if (e == Key.Home) {
